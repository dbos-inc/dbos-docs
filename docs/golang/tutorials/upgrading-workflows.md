...
Enqueued workflows with no application version (for example, enqueued from a [DBOS Client](../reference/client.md) without specifying a version) are only dequeued by processes running the **latest** registered application version.
