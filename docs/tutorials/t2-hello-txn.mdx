...
First, we declared a new static function in the `Hello` class:
```tsx
@OperonTransaction()
static async hello(txnCtxt: TransactionContext, name: string) {
  const greeting = `Hello, ${name}!`
...
Second, we modified the `greetingEndpoint` to invoke the transaction:
```tsx
ctx.invoke(Hello).hello(name).then(x => x.getResult()`
```
