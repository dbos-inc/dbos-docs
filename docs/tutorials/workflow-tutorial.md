...
- [`npx dbos workflow retry <uuid>`](../api-reference/cli.md#npx-dbos-workflow-retry): Retry a workflow from the last step it executed, keeping its workflow identity UUID.
- [`npx dbos workflow restart <uuid>`](../api-reference/cli.md#npx-dbos-workflow-restart): Resubmit a workflow, restarting it from the beginning with the same arguments but a new workflow identity UUID.
