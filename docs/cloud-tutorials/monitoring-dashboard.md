...
The number of total active CPU milliseconds since the start of the month is at the top in orange, and the active CPU milliseconds for the selected region is in light orange. The light orange number changes with the selected app(s) and time window so you can select a particular app or workload and see how much it contributes to your total. 
