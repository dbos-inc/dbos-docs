...
On the bottom of the dashboard, there is a table of trace calls. These are captured handler, workflow, transaction and communicator invocation spans, each timestamped and decorated with duration in milliseconds.
...
The dashboard presents the total number of execution seconds since the start of the month, in orange, and the total for the selected traces in green. You can use the above filtering controls to navigate to any particular app, time interval, group of traces or even a specific trace and use the green number to view its contribution. 
