...
If an exception is thrown from a workflow, the workflow is **considered complete**&mdash;DBOS records the exception, sets the workflow status to `ERROR`, and **does not recover the workflow**.
