...
[Authentication](../development/authentication-authorization.md) in DBOS is done via the middleware function passed to `@Authentication`.
...
except for methods that have no specified [`@RequiredRoles`](../development/authentication-authorization#authorization-decorators).
