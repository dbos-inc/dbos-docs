...
- **workflowIDReusePolicy**: What to do if a workflow with ID `workflowID` already exists, whatever its status. Defaults to `'return-existing'`.
  ...
  - `'reject'`: throw `DBOSWorkflowIDInUseError` without starting a new workflow or modifying the existing one. The error has `workflowID`, `status`, and `workflowName` properties describing the existing workflow. Match it with the `isWorkflowIDInUseError` helper exported in the SDK's `Error` namespace rather than `instanceof`, because an error rethrown from a workflow's recorded history may not be an instance of the class. Cannot be used with a [debouncer](#debouncing).
