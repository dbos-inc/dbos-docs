...
1. use the [CurrentTimeStep](../reference/libraries.md#currenttimestep) to durably retrieve the workflow start time
2. call `getUserAssignment` to retrieve the assignment status for the employee (creating a new assignment if appropriate)
3. use [setEvent](../tutorials/programmingmodel/workflow-communication-tutorial#setevent) to return the assignment status to the caller
